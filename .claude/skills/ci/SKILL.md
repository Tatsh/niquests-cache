...
If any changed files are under `niquests_cache/` or `tests/`, run the following
...
1. **click-auditor** - validate Click command consistency. **Only run if the project directly uses
   Click and files under `niquests_cache/commands/` changed.**
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `niquests_cache/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `niquests_cache/commands/main.py`,
...
For Python files, strip the `niquests_cache/` prefix and replace `/` with `.` (like module imports).

- Python file `niquests_cache/media.py` → `media:`.
- Multiple files under `niquests_cache/commands/` → `commands:`.
- Single command file `niquests_cache/commands/admin.py` → `commands.admin:`.
